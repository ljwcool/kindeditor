...
// import "@/../static/kin/themes/default/default.css";
import "@/../static/kin//kindeditor-all-min.js";
import "@/../static/kin/lang/zh-CN.js";
...
export default {
  ...
  watch: {
    ...
    outContent(val) {
      ...
      console.log("val11222", this.editor.count("text"));

      this.$emit("update:content", val);
      this.$emit("on-content-change", val);
      this.$emit("input", val);
      this.$emit("count", this.editor.count("text"));
    },
    // 初始化编辑器内容
    html(val) {
      if (
  ...
  methods: {
    //统计字数
    getCounts() {
      let count = this.editor.count(this.outContent);
      console.log("111字数", count);
    },
    // 打开文件
    handleOpenFile() {
